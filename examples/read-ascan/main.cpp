...
int main(int argc_, char** argv_)
{
  ...
  NDE::TResult<bool> fileResult = NDE::Validate(jsonSetup, schemaPathFilename);
...
AscanData ReadAscanData(hid_t fileId_, const rj::Value& ascanDataset_)
{
  ...
  ascanData.AmplitudeRange.PercentMin = amplitude["dataValue"]["min"].GetDouble();
  ascanData.AmplitudeRange.PercentMax = amplitude["dataValue"]["max"].GetDouble();
  ascanData.AmplitudeRange.SamplingMin = amplitude["dataSampling"]["min"].GetInt64();
  ascanData.AmplitudeRange.SamplingMax = amplitude["dataSampling"]["max"].GetInt64();

  for (const auto& dimension : ascanDataset_["dimensions"].GetArray())
  {
...
std::vector<Color> CreateColorPalette()
{
  ...
  colors.emplace_back(184, 212, 244);
  colors.emplace_back(113, 170, 233);
  colors.emplace_back(62, 105, 190);
  colors.emplace_back(14, 37, 143);
  colors.emplace_back(27, 72, 129);
  colors.emplace_back(59, 140, 127);
  colors.emplace_back(126, 187, 94);
  colors.emplace_back(211, 223, 45);
  colors.emplace_back(241, 211, 43);
  colors.emplace_back(222, 156, 80);
  colors.emplace_back(209, 121, 87);
  colors.emplace_back(205, 116, 49);
  colors.emplace_back(194, 98, 23);
  colors.emplace_back(167, 50, 26);
  colors.emplace_back(145, 12, 29);

  size_t interpolationPoints(7);
  double f = 1. / interpolationPoints;
  ...
  for (size_t colorIdx(0); colorIdx < colors.size() - 1; colorIdx++)
  {
    ...
    for (size_t j(0); j < interpolationPoints; j++)
    {
      ...
      everyColors.emplace_back(r, g, b);
    }
  }
